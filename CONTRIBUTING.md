...
CI baseline runtime: **Python 3.11**.
...
Repository navigation files under `docs/` (such as `README.md` in domain folders) are still docs and must include valid front matter.
...
## Artifact policy (`dist/`)

- `dist/releases/<version>/` contains immutable, tracked release snapshots.
- `dist/latest/` mirrors the newest release snapshot.
- Do not hand-edit release artifacts. They are generated by the release workflow.
- Local development builds should use non-release versions (for example: `make build VERSION=dev-local`) and write to gitignored `dist/dev-local/`.

## Release policy (automated)

Release creation is automated via workflow dispatch in `.github/workflows/release.yml`.
The workflow validates docs, generates artifacts, commits `dist/` changes, creates tag `corpus-vYYYY.MM.patch`, and publishes a GitHub release.
